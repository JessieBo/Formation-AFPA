...
try {
    if (!isset($_GET["action"])) {
        ...
        if ($_GET["action"] == "verification") { // Vérifier si le login et mdp sont dans la bdd
            verif_connexion();
        } else if ($_GET["action"] == "tableau") { // Affichage de la page tableau.php
            liste_clients();
        } else if ($_GET["action"] == "recherche") { // Recherche d'une société
            recherche_client($_POST["search"]);
